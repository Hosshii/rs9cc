...
impl FromStr for Operator {
    ...
    fn from_str(s: &str) -> Result<Operator, Self::Err> {
        match s {
            x if x.as_bytes()[0] == Plus.as_char() as u8 => Ok(Plus),
            x if x.as_bytes()[0] == Minus.as_char() as u8 => Ok(Minus),
            x if x.as_bytes()[0] == Mul.as_char() as u8 => Ok(Mul),
            x if x.as_bytes()[0] == Div.as_char() as u8 => Ok(Div),
            x if x.as_bytes()[0] == LParen.as_char() as u8 => Ok(LParen),
            x if x.as_bytes()[0] == RParen.as_char() as u8 => Ok(RParen),
